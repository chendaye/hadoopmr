...
/**
 * todo: combiner 本质上就是对 map输出 <key, <value1, value2, value3>> 进一步处理
 */
public class InvertedIndexCombiner extends Reducer<Text, Text, Text, Text> {
    ...
    public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        ...
        info.set(key.toString().substring(splitIndex + 1) + ":" + sum);	//将key中冒号后的内容（行号）与单词数总和组合成Combiner输出的value
        key.set(key.toString().substring(0, splitIndex));				//将key中冒号前的内容（单词）设置为Combiner输出的key
        context.write(key, info);		//输出格式："word	行号:sum"
    }
}
